...
export default class PhoneInput extends Component {
  ...
  updateFlagAndFormatNumber(number, actionAfterSetState = null) {
    ...
    if ((phoneNumber.match(/\+/g) || []).length < 2) { /// It allows only one + sign for phone input
      if (phoneNumber.length > dialCode.length) {
        this.setState({ iso2, formattedNumber: PhoneNumber.format(phoneNumber, this.state.iso2) }, actionAfterSetState);
      } else {
  ...
  render() {
    ...
    return (
      <View style={[{width:this.props.style.width}]}>
        <View style={{marginLeft:styles.flag.width+(this.props.offset || 20)}} > 
