...
const DIAL = function (code) { return `+${code} ` };
...
export default class PhoneInput extends Component {
  ...
  constructor(props, context) {
    ...
    this.state = {
      ...
      countryData,
      disabled,
      formattedNumber: DIAL(countryData.dialCode),
      value: null
    };
  }
  ...
  getPickerData() {
    return PhoneNumber.getAllCountries().map((country, index) => ({
      ...
      label: country.name,
      dialCode: DIAL(country.dialCode),
      iso2: country.iso2
    }));
  }
  ...
  getCountryCode() {
    return this.getCountry().dialCode;
  }
  ...
  getNumber() {
    let dialCode = DIAL(this.state.countryData.dialCode);
    return this.state.formattedNumber && this.state.formattedNumber.replace(dialCode, '');
  }
    ...
    if (this.state.iso2 !== iso2) {
      ...
      if (countryData) {
        this.setState(
          ...
          () => {
            if (this.props.onSelectCountry) this.props.onSelectCountry(iso2, countryData);
          }
        );
      }
    }
  ...
  updateFlagAndFormatNumber(number) {
    ...
    let iso2 = this.state.iso2;
    let formattedNumber;
    let dialCode = this.state.countryData.dialCode;

    if (!number || number.indexOf(DIAL(dialCode)) < 0) {
      this.setState({ formattedNumber: DIAL(dialCode) });
      return;
    }

    if (number) {
      formattedNumber = allowZeroAfterCountryCode
        ? number
        : this.possiblyEliminateZeroAfterCountryCode(number);

      this.setState({ formattedNumber }, () => {
        this.props.onChangePhoneNumber(
          formattedNumber.replace(DIAL(dialCode), ''),
  ...
  render() {
    const { renderCountryPicker } = this.props;
